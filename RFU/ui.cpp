...
LRESULT CALLBACK WindowProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    if (uMsg == RFU_TRAYICON)
    {
        if (lParam == WM_RBUTTONDOWN || lParam == WM_LBUTTONDOWN)
        {
            ...
            CheckMenuRadioItem(submenu, RFU_FCS_FIRST, RFU_FCS_FIRST + Settings::FPSCapValues.size(),
                               RFU_FCS_FIRST + Settings::FPSCapSelection, MF_BYCOMMAND);
            AppendMenu(popup, MF_POPUP, reinterpret_cast<UINT_PTR>(submenu), L"FPS Cap");

            HMENU unlock_method = CreatePopupMenu();
            AppendMenu(unlock_method, MF_STRING, RFU_TRAYMENU_UM_HYBRID, L"Hybrid");
            AppendMenu(unlock_method, MF_STRING, RFU_TRAYMENU_UM_MEMWRITE, L"Memory Write");
            AppendMenu(unlock_method, MF_STRING, RFU_TRAYMENU_UM_FLAGSFILE, L"Flags File");
            CheckMenuRadioItem(unlock_method, RFU_TRAYMENU_UM, RFU_TRAYMENU_UM_LAST,
                               RFU_TRAYMENU_UM + static_cast<uint32_t>(Settings::UnlockMethod), MF_BYCOMMAND);
            AppendMenu(popup, MF_POPUP, reinterpret_cast<UINT_PTR>(unlock_method), L"Unlock Method");
            ...
            const auto result = TrackPopupMenu(popup, TPM_RETURNCMD | TPM_TOPALIGN | TPM_LEFTALIGN, position.x,
...
#pragma warning( pop )
                ...
                case RFU_TRAYMENU_GITHUB:
                    ShellExecuteA(nullptr, "open", "https://github.com/" RFU_GITHUB_REPO, nullptr, nullptr,
                                  SW_SHOWNORMAL);
                    break;
                ...
                default:
                        ...
                        && result <= RFU_FCS_FIRST + Settings::FPSCapValues.size())
                    {
                        ...
                        // NOLINT(clang-diagnostic-implicit-int-conversion)
                        Settings::FPSCap = Settings::FPSCapSelection == 0
                                               ? 0.0
                                               : Settings::FPSCapValues[Settings::FPSCapSelection - 1];
                    }
